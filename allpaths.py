...
##########################################################
def path_calculculation  (vertex,Edges_in):
    nodes=[]
    Edges=[]
    All_weight=[]
    Path_Names=[]
    ...
    for i in range (0,len(path)):
            ...
            for j in range (0,len(path_name)-1):
            	path_edge.append((path_name[j],path_name[j+1]))

            temp.append(path_edge)
            temp.append(weight)
            Path_Dict[i]=temp
...
if __name__ == '__main__':
    ...
    path_calculculation (vertex,Edges_in)  
    print "done"           